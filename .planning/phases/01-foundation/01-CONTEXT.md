...
- **D-20 (RECONCILIATION — added 2026-04-27 after research):** The `enrollments` table already exists from migration `0001_initial_schema.sql` with columns `id`, `user_id`, `course_id`, `order_id` (FK orders — payment feature is out-of-scope), `status` (enum `enrollment_status` ACTIVE/INACTIVE/COMPLETED), `created_at`. UNIQUE `(user_id, course_id)` is already in place. **Strategy: additive ALTER in 0013** — `ADD COLUMN source enrollment_source NOT NULL DEFAULT 'admin_grant'`, `ADD COLUMN granted_at timestamptz NOT NULL DEFAULT now()` (alongside `created_at` — do not rename), `ADD COLUMN expires_at timestamptz NULL`, `ADD COLUMN institution_id uuid NULL REFERENCES institutions(id) ON DELETE RESTRICT`. KEEP `status` and `order_id` columns as legacy/deprecated (no code writes to enrollments yet, so backfill writes only the new columns; legacy rows take the default `status='ACTIVE'` and `order_id=NULL`). New `enrollment_source` enum is created in 0013 (not in 0012 — 0012 is enum-value-only for `user_role`). RLS policies in D-08 reference only the new columns; ENR-02 active-enrollment check uses `expires_at IS NULL OR expires_at > now()`, not the legacy `status`.
