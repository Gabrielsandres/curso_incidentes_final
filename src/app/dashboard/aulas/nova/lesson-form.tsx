...
function sortCourseOptions(options: CourseSummary[]) {
  return [...options].sort((left, right) => left.title.localeCompare(right.title, "pt-BR"));
}
...
export function CreateLessonForm({
  ...
  const filteredModuleOptions = useMemo(
    () => moduleOptions.filter((moduleOption) => moduleOption.courseId === selectedCourseId),
    [moduleOptions, selectedCourseId],
  );

  const hasModulesForSelectedCourse = filteredModuleOptions.length > 0;
  ...
  useEffect(() => {
    ...
    const currentSelectionStillValid = filteredModuleOptions.some((moduleOption) => moduleOption.id === selectedModuleId);
    if (!currentSelectionStillValid) {
      setSelectedModuleId(filteredModuleOptions[0].id);
    }
  }, [filteredModuleOptions, selectedCourseId, selectedModuleId]);
    ...
    <>
      ...
      >
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Formulario</p>
          <h2 className="text-xl font-semibold text-slate-900">Dados da aula</h2>
          <p className="text-sm text-slate-600">
            Selecione primeiro o curso, depois o modulo, e preencha as informacoes obrigatorias. Titulo, link do
            video e posicao sao campos obrigatorios.
          </p>
        </div>

        {!courseOptions.length ? (
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
            Nenhum curso cadastrado. Crie um curso em <strong>Gerenciar cursos</strong> antes de cadastrar aulas.
          </div>
        ) : null}
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <label className="flex flex-col gap-2">
            <span className="text-sm font-medium text-slate-700">Selecionar curso *</span>
            <select
              name="course_id"
              required
            ...
            <FieldError errors={lessonState.fieldErrors?.courseId} />
          </label>

          <label className="flex flex-col gap-2">
            <span className="text-sm font-medium text-slate-700">Selecionar modulo *</span>
            <select
              name="module_id"
              required
              value={selectedModuleId}
              disabled={!selectedCourseId || !hasModulesForSelectedCourse}
              onChange={(event) => {
            ...
            >
              <option value="" disabled>
                {!selectedCourseId
                  ...
                  : hasModulesForSelectedCourse
                    ? "Selecione um modulo"
                    : "Este curso ainda nao possui modulo"}
              </option>
              {filteredModuleOptions.map((moduleOption) => (
